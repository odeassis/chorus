...
## Purpose
Guarantee that no Chorus deployment silently signs session JWTs with a publicly known
default `NEXTAUTH_SECRET`: the Docker entrypoint generates and persists a random secret
when none (or a known placeholder) is configured, and the application loudly warns when a
known placeholder is still in effect.
